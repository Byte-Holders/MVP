...
function createHelperMock(): jest.Mocked<DependencyNodeHelper> {
  return {
    ...
    translateDescriptions: jest
      .fn()
      .mockResolvedValue(TRANSLATED_VULNERABILITIES),
...
describe('DependencyNodeService', () => {
  ...
  describe('scan', () => {
    it('should orchestrate all helper steps in the correct order', async () => {
      ...
      helper.executeSyft.mockImplementation(async () => {
        callOrder.push('executeSyft');
        return '{}';
      });
      helper.parseSbom.mockImplementation(() => {
        callOrder.push('parseSbom');
        return DEPS_LIST;
      });
      helper.executeGrype.mockImplementation(async () => {
        callOrder.push('executeGrype');
        return '{}';
      });
      helper.parseGrype.mockImplementation(() => {
        callOrder.push('parseGrype');
        return RAW_VULNERABILITIES;
      });
      helper.translateDescriptions.mockImplementation(async () => {
        callOrder.push('translateDescriptions');
        return TRANSLATED_VULNERABILITIES;
      });
      helper.analyzeDependencies.mockImplementation(async () => {
    ...
    it('should map vulnerability fields correctly into the report', async () => {
      ...
      expect(
        (result.depsReport as any).vulnerabilities[0].fixVersion,
      ).toBeUndefined();
    });
  });
  ...
  describe('scan – grype failure', () => {
    beforeEach(() => {
      helper.executeGrype.mockRejectedValue(
        new Error('grype binary not found'),
      );
    });
  ...
  describe('scan – analyzeDependencies failure', () => {
    it('should propagate the error and return the default report', async () => {
      helper.analyzeDependencies.mockRejectedValue(
