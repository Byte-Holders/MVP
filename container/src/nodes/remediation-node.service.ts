...
export class RemediationNodeService {
  ...
  async Scan(
    state: WorkflowState & { semgrepReportPath?: string },
    ...
    const enrichedUnits: VulnerabilityUnit[] = [
      ...(state.vulnerabilitiesReport?.report ?? []),
    ];
      ...
      [...byFile.entries()].map(async ([filePath, results]) => {
        ...
        try {
          ...
          const content = (response.content as string)
            .replace(/```json|```/g, '')
            .trim();
          const parsed: { id: string; remediation: string }[] =
            JSON.parse(content);
