...
describe('WorkspaceController (e2e)', () => {
  ...
  describe('DELETE /workspaces/:workspaceId/repositories/:id', () => {
    it('dovrebbe rimuovere una repository dal workspace e restituire 200', async () => {
      const workspaceAggiornato =
        await workspaceModel.findById(workspaceRealeId);
      ...
      const response = await request(app.getHttpServer() as any).delete(
        `/api/workspaces/${workspaceRealeId}/repositories/${repoIdDaCancellare}`,
      );
