...
export class WorkspaceManagerService implements IWorkspaceManagerService {
  ...
  async createWorkspace(dto: CreateWorkspaceDto, authenticatedUsername: string): Promise<CreateWorkspaceResponseDto>  {
    ...
    return this.repository.create(dto, authenticatedUsername)*/
    const workspace = await this.repository.create(dto, authenticatedUsername)
    ...
    response.createdBy = authenticatedUsername
