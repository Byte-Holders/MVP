...
export class WorkspaceDocumentMapper {
  static toBO(doc: WorkspaceDocument): WorkspaceBo {
    ...
    bo.ownerUsername =
      bo.members.find((m) => m.userId === doc.ownerId)?.username ?? '';
  ...
  static toListItemBo(
    ...
    bo.name = doc.name;
    bo.owner =
      doc.members.find((m) => m.userId === doc.ownerId)?.userUsername ?? '';
    bo.role =
      doc.members.find((m) => m.userId === requesterId)?.role ?? 'viewer'; // se non trova il membro, assegna ruolo viewer (caso raro, ma meglio gestirlo)
    return bo;
  }
}
