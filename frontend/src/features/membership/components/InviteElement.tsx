...
export const InviteElement: React.FC<Props> = ({ invite, onAction }) => {
  ...
  const workspaceInitials = invite.workspaceId ? invite.workspaceId.substring(0, 2).toUpperCase() : 'WS';
  ...
  return (
    <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between rounded-xl border border-[var(--h-line)] bg-[var(--surface)] p-5 shadow-sm transition-all hover:border-[var(--sea-ink-soft)]/30 hover:shadow-md">
      ...
      <div className="flex items-center gap-4 mb-4 sm:mb-0">
        ...
        <div>
          ...
          <p className="text-sm text-[var(--sea-ink-soft)]">
            Sei stato invitato da: <span className="font-medium text-[var(--sea-ink)]">{invite.senderUsername}</span>
          </p>
        </div>
      </div>
      ...
      <div className="flex w-full sm:w-auto gap-3">
        <button 
          onClick={() => handleReject(invite.recipientUsername, invite.workspaceId)} 
        ...
        <button 
          onClick={() => handleAccept(invite.recipientUsername, invite.workspaceId)} 
