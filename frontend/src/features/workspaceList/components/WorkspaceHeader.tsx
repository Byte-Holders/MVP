...
import { DeleteWorkspaceButton } from '#/features/deleteWorkspace/components/DeleteWorkspaceButton'
import { useAuth } from '@/features/auth/hooks/useAuth' // 👈 AGGIUNGI QUESTO
...
export function WorkspaceHeader({ workspaceId }: Props) {
  ...
  const { user: currentUser } = useAuth() // 👈 UTENTE LOGGATO
  ...
  const isOwner =
    workspace.owner && currentUser && workspace.owner === currentUser.username
          ...
          <div className="flex items-start gap-5">
            ...
            <div className="flex flex-col gap-2 min-w-0">
              <div className="flex flex-wrap items-center gap-2">
                ...
                <span
                  className={`rounded-full border px-3 py-0.5 text-xs font-semibold ${roleConfig(workspace.role).bg} ${roleConfig(workspace.role).text} ${roleConfig(workspace.role).border}`}
                >
